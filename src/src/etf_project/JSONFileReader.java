...
public class JSONFileReader {
	...
	final int DESK_SIGN_START = 4;
	final int DESK_SIGN_END = 6;
	final int SCHOOL_YEAR_SIGN = 6;
	final int SUBJECT_CODE_START = 7;
	...
	public List<Exam> readExams(String filePath, int numOfDays){
		...
		try {
			...
			for(int i = 0; i < examList.length(); i++) {
				...
				int accreditation = Integer.parseInt(examCode.substring(ACCREDITATION_SIGN_START, ACCREDITATION_SIGN_END));
				String department = examCode.substring(DEPARTMENT_SIGN, DEPARTMENT_SIGN+2);
				...
				Exam newExam = new Exam(numOfDays, accreditation, department, desk, schoolYear, subject, newExamJSON.getInt("prijavljeni"),onComputer, departmentSign);
				exams.add(newExam);
			}
	...
	public List<Classroom> readClassRooms(String filePath, int numOfDays){
		...
		try {
			...
			Classroom newCR = new Classroom(numOfDays, newObject.getJSONObject(i).getString("naziv"), newObject.getJSONObject(i).getInt("kapacitet"), hasComputers, newObject.getJSONObject(i).getInt("dezurni"), belongsToETF);
